...
import { SourceInfoModel, SourceInfoModelType } from "./SourceInfoModel"
import { sourceInfoModelPrimitives, SourceInfoModelSelector } from "./SourceInfoModel.base"
...
export const RootStoreBase = withTypedRefs<Refs>()(MSTGQLStore
  ...
  .actions(self => ({
    ...
    queryValidateSource(variables: { data: SourceValidationParams }, resultSelector: string | ((qb: SourceInfoModelSelector) => SourceInfoModelSelector) = sourceInfoModelPrimitives.toString(), options: QueryOptions = {}) {
      return self.query<{ validateSource: SourceInfoModelType[]}>(`query validateSource($data: SourceValidationParams!) { validateSource(data: $data) {
        ${typeof resultSelector === "function" ? resultSelector(new SourceInfoModelSelector()).toString() : resultSelector}
      } }`, variables, options)
    },
