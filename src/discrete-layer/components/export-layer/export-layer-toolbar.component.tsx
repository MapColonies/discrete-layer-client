...
const ExportLayerToolbar: React.FC<ExportLayerToolbarProps> = observer(() => {
  ...
  const ToggleActionPresentor: React.FC<ActionPresentorBaseProps> = ({ action, key, data }) => {
      const intl = useIntl();
      if (typeof action.toggleExportStoreFieldOptions === "undefined") return null;

      const toggleOptions = action.toggleExportStoreFieldOptions;
      const isChecked = get(exportStore, toggleOptions.field) as boolean;
      const checkboxLabel = isChecked ? intl.formatMessage({ id: toggleOptions.labelChecked }) : intl.formatMessage({ id: toggleOptions.labelUnchecked });
